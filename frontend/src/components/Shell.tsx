    ...
    <div className="min-h-screen flex flex-col">
        ...
        <div className="max-w-6xl mx-auto px-4 h-9 flex items-center justify-between">
          <div className="flex items-center gap-5">
            ...
            {settings?.working_hours_note && (
              <span className="hidden sm:block text-gray-600 border-l border-gray-800 pl-5">
                {settings.working_hours_note}
              </span>
            )}
          </div>
        ...
        <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            {user ? (
              <>
                <NotificationBell />
                <Link to="/patient" className="text-sm font-medium text-blue-600 hover:underline">
                  {user.full_name}
                </Link>
              ...
              <>
                <Link to="/login" className="btn-secondary text-sm py-1.5">Увійти</Link>
                <Link to="/login?tab=register" className="btn-primary text-sm py-1.5">Реєстрація</Link>
              </>
            )}
          </div>
        </div>
      ...
      <footer className="bg-gray-950 text-gray-400 text-sm">
        ...
        <div className="border-t border-gray-800 py-4 text-center text-xs text-gray-600">
          © {new Date().getFullYear()} DentaCare. Всі права захищені.
        </div>
      </footer>
    </div>
  );
