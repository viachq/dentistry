...
# ── Database (Docker) ──
if (Test-Port 5435) {
    Write-Host "[OK] PostgreSQL already on port 5435" -ForegroundColor Green
} else {
    ...
    do {
        ...
        $result = docker compose -f "$root\docker-compose.yml" exec db pg_isready -U postgres -d dentistry 2>&1
    } while ($result -notmatch "accepting connections" -and $attempt -lt 30)

    if ($attempt -ge 30) {
        Write-Host "[FAIL] PostgreSQL did not start." -ForegroundColor Red
        exit 1
    }
    Write-Host "[OK] PostgreSQL ready on port 5435" -ForegroundColor Green
}
...
Write-Host ""
Write-Host "=== DentaCare ===" -ForegroundColor Green
Write-Host "  PostgreSQL  localhost:5435"
